...
```mermaid
graph LR
    start[输入词汇] --> encode[词汇编码]
    encode --> generate[使用编码后toke_id数组进行推理]
    generate --> conditionforward{判断是否到推理终止}
    conditionforward -- YES --> output{输出推理结果}
    conditionforward -- NO --> forward{进行推理}
    forward -->  conditionforward
```

```mermaid
graph LR
    forward[输入token_id数组] --> embedding[把每个token_id转换为向量]
    embedding--> run[模型运算]
    run--> condition{判断是否进行到模型的最后一层}
    condition -- YES --> output{输出推理结果}
    condition -- NO --> rms[对embedding后的数据归一化]
    rms --> qkv[计算q,k,v]
